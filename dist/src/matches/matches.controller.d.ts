import { MatchesService } from './matches.service';
import { FixtureService } from './fixture.service';
import { CargarResultadoDto } from './dto/cargar-resultado.dto';
export declare class MatchesController {
    private readonly matchesService;
    private readonly fixtureService;
    constructor(matchesService: MatchesService, fixtureService: FixtureService);
    generarFixture(tournamentId: string): Promise<{
        tournamentId: string;
        fixtures: any[];
        message: string;
    }>;
    obtenerFixture(tournamentId: string, categoryId?: string): Promise<{}>;
    findOne(id: string): Promise<{
        category: {
            id: string;
            nombre: string;
            createdAt: Date;
            tipo: import(".prisma/client").$Enums.Gender;
            orden: number;
        };
        tournament: {
            id: string;
            nombre: string;
            descripcion: string | null;
            createdAt: Date;
            ciudad: string;
            estado: import(".prisma/client").$Enums.TournamentStatus;
            updatedAt: Date;
            pais: string;
            region: string;
            fechaInicio: Date;
            fechaFin: Date;
            flyerUrl: string;
            costoInscripcion: import("@prisma/client/runtime/library").Decimal;
            sede: string | null;
            direccion: string | null;
            mapsUrl: string | null;
            fechaLimiteInscr: Date;
            organizadorId: string;
        };
        cancha: {
            complejo: {
                id: string;
                nombre: string;
                createdAt: Date;
                direccion: string | null;
                mapsUrl: string | null;
                tournamentId: string;
                esSedePrincipal: boolean;
            };
        } & {
            id: string;
            nombre: string;
            createdAt: Date;
            tipo: string;
            estado: string;
            complejoId: string;
        };
        pareja1: {
            jugador1: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
            jugador2: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            jugador2Documento: string;
            jugador1Id: string;
            jugador2Id: string | null;
        };
        pareja2: {
            jugador1: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
            jugador2: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            jugador2Documento: string;
            jugador1Id: string;
            jugador2Id: string | null;
        };
        parejaGanadora: {
            jugador1: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
            jugador2: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            jugador2Documento: string;
            jugador1Id: string;
            jugador2Id: string | null;
        };
    } & {
        id: string;
        createdAt: Date;
        estado: import(".prisma/client").$Enums.MatchStatus;
        updatedAt: Date;
        categoryId: string;
        tournamentId: string;
        set1Pareja1: number | null;
        set1Pareja2: number | null;
        set2Pareja1: number | null;
        set2Pareja2: number | null;
        set3Pareja1: number | null;
        set3Pareja2: number | null;
        parejaGanadoraId: string | null;
        observaciones: string | null;
        ronda: string;
        numeroRonda: number;
        pareja1Id: string | null;
        pareja2Id: string | null;
        canchaId: string | null;
        fechaProgramada: Date | null;
        horaProgramada: string | null;
        horaFinEstimada: string | null;
        parejaPerdedoraId: string | null;
        partidoSiguienteId: string | null;
    }>;
    cargarResultado(id: string, dto: CargarResultadoDto): Promise<{
        category: {
            id: string;
            nombre: string;
            createdAt: Date;
            tipo: import(".prisma/client").$Enums.Gender;
            orden: number;
        };
        tournament: {
            id: string;
            nombre: string;
            descripcion: string | null;
            createdAt: Date;
            ciudad: string;
            estado: import(".prisma/client").$Enums.TournamentStatus;
            updatedAt: Date;
            pais: string;
            region: string;
            fechaInicio: Date;
            fechaFin: Date;
            flyerUrl: string;
            costoInscripcion: import("@prisma/client/runtime/library").Decimal;
            sede: string | null;
            direccion: string | null;
            mapsUrl: string | null;
            fechaLimiteInscr: Date;
            organizadorId: string;
        };
        cancha: {
            complejo: {
                id: string;
                nombre: string;
                createdAt: Date;
                direccion: string | null;
                mapsUrl: string | null;
                tournamentId: string;
                esSedePrincipal: boolean;
            };
        } & {
            id: string;
            nombre: string;
            createdAt: Date;
            tipo: string;
            estado: string;
            complejoId: string;
        };
        pareja1: {
            jugador1: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
            jugador2: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            jugador2Documento: string;
            jugador1Id: string;
            jugador2Id: string | null;
        };
        pareja2: {
            jugador1: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
            jugador2: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            jugador2Documento: string;
            jugador1Id: string;
            jugador2Id: string | null;
        };
        parejaGanadora: {
            jugador1: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
            jugador2: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            jugador2Documento: string;
            jugador1Id: string;
            jugador2Id: string | null;
        };
    } & {
        id: string;
        createdAt: Date;
        estado: import(".prisma/client").$Enums.MatchStatus;
        updatedAt: Date;
        categoryId: string;
        tournamentId: string;
        set1Pareja1: number | null;
        set1Pareja2: number | null;
        set2Pareja1: number | null;
        set2Pareja2: number | null;
        set3Pareja1: number | null;
        set3Pareja2: number | null;
        parejaGanadoraId: string | null;
        observaciones: string | null;
        ronda: string;
        numeroRonda: number;
        pareja1Id: string | null;
        pareja2Id: string | null;
        canchaId: string | null;
        fechaProgramada: Date | null;
        horaProgramada: string | null;
        horaFinEstimada: string | null;
        parejaPerdedoraId: string | null;
        partidoSiguienteId: string | null;
    }>;
    reprogramar(id: string, body: {
        fechaProgramada: string;
        horaProgramada: string;
        canchaId?: string;
    }): Promise<{
        id: string;
        createdAt: Date;
        estado: import(".prisma/client").$Enums.MatchStatus;
        updatedAt: Date;
        categoryId: string;
        tournamentId: string;
        set1Pareja1: number | null;
        set1Pareja2: number | null;
        set2Pareja1: number | null;
        set2Pareja2: number | null;
        set3Pareja1: number | null;
        set3Pareja2: number | null;
        parejaGanadoraId: string | null;
        observaciones: string | null;
        ronda: string;
        numeroRonda: number;
        pareja1Id: string | null;
        pareja2Id: string | null;
        canchaId: string | null;
        fechaProgramada: Date | null;
        horaProgramada: string | null;
        horaFinEstimada: string | null;
        parejaPerdedoraId: string | null;
        partidoSiguienteId: string | null;
    }>;
    obtenerPendientes(tournamentId: string): Promise<({
        category: {
            id: string;
            nombre: string;
            createdAt: Date;
            tipo: import(".prisma/client").$Enums.Gender;
            orden: number;
        };
        cancha: {
            complejo: {
                id: string;
                nombre: string;
                createdAt: Date;
                direccion: string | null;
                mapsUrl: string | null;
                tournamentId: string;
                esSedePrincipal: boolean;
            };
        } & {
            id: string;
            nombre: string;
            createdAt: Date;
            tipo: string;
            estado: string;
            complejoId: string;
        };
        pareja1: {
            jugador1: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
            jugador2: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            jugador2Documento: string;
            jugador1Id: string;
            jugador2Id: string | null;
        };
        pareja2: {
            jugador1: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
            jugador2: {
                id: string;
                nombre: string;
                createdAt: Date;
                documento: string;
                email: string;
                telefono: string;
                apellido: string;
                genero: import(".prisma/client").$Enums.Gender;
                passwordHash: string;
                fechaNacimiento: Date | null;
                ciudad: string | null;
                bio: string | null;
                fotoUrl: string | null;
                estado: import(".prisma/client").$Enums.UserStatus;
                emailVerificado: boolean;
                esPremium: boolean;
                ultimaSesion: Date | null;
                updatedAt: Date;
            };
        } & {
            id: string;
            createdAt: Date;
            updatedAt: Date;
            jugador2Documento: string;
            jugador1Id: string;
            jugador2Id: string | null;
        };
    } & {
        id: string;
        createdAt: Date;
        estado: import(".prisma/client").$Enums.MatchStatus;
        updatedAt: Date;
        categoryId: string;
        tournamentId: string;
        set1Pareja1: number | null;
        set1Pareja2: number | null;
        set2Pareja1: number | null;
        set2Pareja2: number | null;
        set3Pareja1: number | null;
        set3Pareja2: number | null;
        parejaGanadoraId: string | null;
        observaciones: string | null;
        ronda: string;
        numeroRonda: number;
        pareja1Id: string | null;
        pareja2Id: string | null;
        canchaId: string | null;
        fechaProgramada: Date | null;
        horaProgramada: string | null;
        horaFinEstimada: string | null;
        parejaPerdedoraId: string | null;
        partidoSiguienteId: string | null;
    })[]>;
}
