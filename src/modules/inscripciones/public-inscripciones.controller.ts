...
export class PublicInscripcionesController {
  ...
  private validarReglasCategoria(
    ...
    // REGLA 2: Categoría inferior (menor orden = categoría más alta)
    // Ej: Si jugador es 3ra (orden 3), no puede jugar 4ta (orden 4) inferior
    if (ordenTarget > ordenJugador) {
      if (jugadorGenero === 'MASCULINO') {
        ...
        // Mujeres pueden UNA categoría inferior como excepción
        ...
        return {
          ...
          esCategoriaInferior: true,
          advertencia: 'Estás usando tu excepción de una categoría inferior. Esta acción solo puede realizarse una vez.',
        };
      }
    }

    // Categoría igual o superior - permitida para todos
    return {
      permitido: true,
